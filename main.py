...
def search_pptx_for_placeholder(prs, placeholder):
    """Search for placeholder in all slides' shapes of a PPTX presentation"""
...
with tab1:
    ...
    if excel_file:
        ...
        # Changed default marker color to red
        ...
        if palette_option == "Shades of Blue (Light to Dark)":
            # Smooth gradient from light to dark blue
            colors = [
        ...
        try:
            # Read Overview sheet, starting at A4 (header row)
            overview_df = pd.read_excel(tmp_path, sheet_name="Overview", header=3, usecols="A:G", nrows=13)  # A4:G16
            
            # Exclude the "Year" row and format to 2 decimal places
            ghi_data = overview_df.iloc[:-1].copy()  # Remove last row (Year)
            ghi_data.iloc[:, 1:] = ghi_data.iloc[:, 1:].apply(pd.to_numeric, errors='coerce').round(2)
            
            # Extract months from column A and data from columns B-G
            months = ghi_data.iloc[:, 0].tolist()  # Column A (Month)
            data_values = ghi_data.iloc[:, 1:]  # Columns B:G (data sources)
            
            # Warn about invalid data (e.g., #REF!)
        ...
        try:
            # Read Overview sheet, starting at A20 (header row)
            temp_df = pd.read_excel(tmp_path, sheet_name="Overview", header=19, usecols="A:G", nrows=13)  # A20:G32
            
            # Exclude the "Year" row and format to 2 decimal places
            temp_data = temp_df.iloc[:-1].copy()  # Remove last row (Year)
            temp_data.iloc[:, 1:] = temp_data.iloc[:, 1:].apply(pd.to_numeric, errors='coerce').round(2)
            
            # Extract months from column A and data from columns B-G
            months = temp_data.iloc[:, 0].tolist()  # Column A (Month)
            data_values = temp_data.iloc[:, 1:]  # Columns B:G (data sources)
    ...
    if template_file and st.button("Generate Report"):
        ...
        try:
            if output_format == ".docx":
                ...
                for p in doc.paragraphs:
                    for key, value in st.session_state.generated_assets['recap'].items():
                        if f"{{{key}}}" in p.text:
                            p.text = p.text.replace(f"{{{key}}}", value)
                            st.info(f"Replaced text placeholder {{{key}}} in DOCX")
                ...
                for asset_key in ['chart1', 'chart2', 'chart3', 'chart4', 'chart5', 'table1_img', 'table2_img']:
                    if asset_key in st.session_state.generated_assets:
                        ...
                        # Handle tables differently if native option is selected
                                ...
                                for run in paragraph.runs:
                                    for key, value in st.session_state.generated_assets['recap'].items():
                                        if f"{{{key}}}" in run.text:
                                            ...
                                            st.info(f"Replaced text placeholder {{{key}}} in PPTX slide")
